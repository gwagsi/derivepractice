...
const MAX_TRADES_BEFORE_COOLDOWN = 10;
...
const buyContract = async (symbolValue, risk) => {
  ...
  if (times >= MAX_TRADES_BEFORE_COOLDOWN) {
    ...
    console.log(
      `Reached ${MAX_TRADES_BEFORE_COOLDOWN} trades. Entering 30-minute cooldown period.`
    );
  }

  const baseAmount = 1;
  const compundFactor = (compund[risk] / 10).toFixed(0);

  let buyPrice = baseAmount * Math.pow(1 + 0.01 * risk, compundFactor);
  console.log(
    `Base amount: ${baseAmount}, Compound factor: ${compundFactor}, Buy price before cap: $${buyPrice}`
  );
  ...
  console.log(`Buy price after cap: $${buyPrice}`);
...
const authorizeApp = async () => {
  ...
  try {
    const auth = await api.authorize(user_accounts[0].token);
    console.log(
